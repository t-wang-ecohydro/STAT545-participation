...
gapminder %>% 
  ...
  summarize(min_life = min(lifeExp), 
              country=country[lifeExp==min_life]) %>%  #note how it can use min_life in the same function where min_life was first declared
#order matters (putting "country" first would not work because it tries to call on a min_life that doesn't exist yet)
#mutate is not used because we want to reduce a vector to a single number
  arrange(min_life)
```
...
#  arrange(inc_life_exp) %>% 
  filter(inc_life_exp == min(inc_life_exp)) %>% 
  arrange(inc_life_exp) %>% #actually putting arrange() first gave the same output
  knitr::kable()
```
