...
ggplot(mpg, aes(cty, hwy)) + 
  geom_jitter(alpha=0.5, size=1) + geom_smooth(method="lm")
```

![](cm008-exercise_files/figure-html/unnamed-chunk-2-1.png)<!-- -->

```r
#jitter is random every time it's rendered 
#trendline is fitting to orig. data not jitter
```
...
gapminder %>% 
#  group_by(country) %>% 
  ggplot(aes(year, lifeExp, color = country=="Rwanda")) +
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
  ...
  facet_wrap(~continent, scales="free") #makes a panel for each categorical variable
```

![](cm008-exercise_files/figure-html/unnamed-chunk-4-1.png)<!-- -->

```r
#scales="free" adjusts scale of each facet; can no longer compare
#also can free up each axis individually, i.e. "free_x", "free_y"
```
...
gapminder %>% 
  ...
  geom_point(alpha = 0.5, shape=21) + #21 is a hollow circle
  scale_x_log10(labels = scales::comma_format()) +
  scale_size_area()
```
