...
| scale                 | linear |
| statistical transform | count (countries -> continents) |
...
ggplot(mauna, aes(y = month)) +
  ...
  ylab("CO2") #otherwise y is still labeled as month
```
...
```r
p<-ggplot(mauna, aes(month, conc))+geom_line() #go back to 2(a) and assign p directly there. and also add () around the whole function to make 2(a) still display (or call p afterwards to make it print to screen)
p +
  geom_point(colour = 'green', alpha=0.2) #call geom_line after geom_point to plot the line over top of the points
```
...
```r
polar<-ggplot(mauna, aes(conc, month))+geom_line()
#polar
polar+coord_polar()
```
