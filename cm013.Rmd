...
1. intuitive, doesn't need accompanying captions explaining what things are
1. axes and units clearly labeled
1. all parts of the data are visible (proper scale)
1. colors are visually distinct from one another
1. proper plot type based on data, e.g. scatter vs line vs bar for discrete/continuous data
...
- Very weak--working more hours is objective but you can't comment on motivation based on the graph. Also there are other countries whose workers work even more hours.

- Main takeaway: 
  - 2 claims here, maybe of differing veracity
  - graph is exaggerated--less than 1 hr difference between Germany & the ~6 countries below
  - axis doesn't start at 0
  - no measure of uncertainty/standard error
  - sample size? 
  - what kind of statistical analysis was done--mean, median, etc.?
  - why use gridlines and also add precise numbers?
...
- No--no uncertainty provided, scale and gridline increments too big; impression is: it rises slightly but so what? It barely breaks the 60F mark

- Main takeaway: 
  - F less interpretable than C (?)
  - ~2 degrees increase, completely obscured in graph
  - how many years plotted?
  - you need some scientific knowledge to interpret graphs
  - don't blindly follow advice; think about what you are trying to tell with the graph
...
- Very weak; not just weak but false--y-axis scale is reversed

- Main takeaway:
  - choice of color can influence interpretation
  - don't try to 'be creative' with the plot to the extent that it's deceptive
...
- y-axis: autism scale goes from 0.1% to 0.6%, whereas MMR coverage goes from 86% to 96%--factor of 100 difference
- if you were plotting them on proper axes the autism line would barely move

- authors did a simple linear regression
- but if you continue the regression eventually people will be running in negative time
...
1. apply principle of proportional ink, e.g. don't truncate y-axis
1. maintain high data-to-ink ratio
1. update axis labels and titles
1. choose scale & graph type carefully
1. use accessible & easily readable colors
...
gapminder %>% 
  plot_ly(x = ~gdpPercap, 
          ...
          color = ~continent, 
          type = 'scatter', 
          mode = 'markers') # use "mode = 'line'" for line graphs

# only works in HTML (for knitting) unless you export the img

# How to upload to Plot.ly:
# Sys.setenv("plotly_username"="[YOUR_USERNAME_HERE]")
# Sys.setenv("plotly_api_key"="[API_KEY_HERE]")

# api_create(p, filename = "cm013-plotly-example")

# RStudio will overwrite if a file with the same name exists
```
