...
```{r}
(my_lm <- lm(formula = lifeExp ~ year, data = gapminder_France))
...
# shift y-axis to minimum year in dataset

(my_lm <- lm(lifeExp ~ I(year - 1952), gapminder_France))

#my_lm is a large object, contains a lot of data

```

Use the `unclass()` function to take a look at how the `lm()` object actually looks like.
```{r}
unclass(my_lm) #takes a long time to run, contains a lot of info about stats, residuals, etc.
```
...
# create dataframe that contains a new set of years & use predict to interpolate life expectancy
```
```{r}
plot(my_lm)
```
...
```{r}
years1 = FILL_THIS_IN # see gapminder_France2
